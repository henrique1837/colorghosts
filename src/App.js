...
function App() {
  ...
  const getLastNftsMTEvents = async (from,to,tokenId) => {
    ...
    const obj = {
      img: `${metadataToken.image_data}`,
      name: metadataToken.name,
      velocity: metadataToken.attributes[1].value
    }

    if(!nfts.includes(JSON.stringify(obj))){
      const newNfts = nfts;
      newNfts.unshift(JSON.stringify(obj))
  ...
  return (
    <AppContext.Provider value={{ state, actions }}>
      ...
      >
        ...
        {
          state.nfts?.length > 0 &&
          <NFTs nfts={state.nfts.slice(0,5).map(str => {return(JSON.parse(str))})} title={`Latest GhostNColors minted`} />
        }
        <s.SpacerMedium />
...

        {
          ...
          <NFTs nfts={state.myNfts.slice(0,5).map(str => {return(JSON.parse(str))})} title={`Latest GhostNColors minted by you`} />
        }
